...
function renderCalendar() {
  ...
  for (let i = 0; i < 42; i += 1) {
    ...
    const attentionCount = dayRows.filter(needsAttention).length;
...
function renderList() {
  ...
  const selectedReasons = selectedIncompleteFilterKeys();
  const descriptions = {
    personstore: '担当者と拠点で予定を絞り込めます。',
    incomplete: selectedReasons.length ? `選択した不足理由のいずれかに該当する予定です：${selectedReasons.map((key) => INCOMPLETE_FILTER_LABELS[key] || key).join('・')}` : '未確定ステータス、または必須項目が不足している予定です。',
  };
...
function scheduleActionButtons(schedule) {
  const buttons = [];
  if (canEditSchedule()) buttons.push(`<button type="button" class="btn small" data-edit-schedule="${escapeHtml(schedule.id)}">予定を編集</button>`);
  if (canEditMemo()) buttons.push(`<button type="button" class="btn small" data-edit-memo="${escapeHtml(schedule.id)}">メモ変更</button>`);
  if (canEditStartTime()) buttons.push(`<button type="button" class="btn small primary" data-edit-start-time="${escapeHtml(schedule.id)}">開始時間変更</button>`);
  return buttons.length ? `<div class="card-actions">${buttons.join('')}</div>` : '';
}
...
function scheduleDetailCard(schedule) {
  ...
  const files = canReadFiles() && schedule.files?.length ? schedule.files.map((file) => {
    const label = file.category === 'attachment' ? '添付ファイル' : (extensionFileLabel(file) || '資料');
    return `<div class="file-box"><div class="file-head"><strong>${escapeHtml(label)}</strong><a class="file-link" href="/files/${encodeURIComponent(file.id)}" target="_blank" rel="noopener">${escapeHtml(file.filename)}</a></div>${canDeleteFiles() ? `<div class="file-actions"><button type="button" class="btn small danger" data-delete-file="${escapeHtml(file.id)}">ファイル削除</button></div>` : ''}</div>`;
  }).join('') : '';
  return `<article class="schedule-card" style="--company-color:${escapeHtml(colors[0])}"><div class="title-line"><h3>${escapeHtml(schedule.startTime || '時刻未設定')}　${escapeHtml(scheduleTypeName(schedule.scheduleTypeId))}</h3><span class="status-chip ${workflowStatusClass(status)}">${escapeHtml(workflowStatusLabel(status))}</span></div>${missing.length ? `<div class="incomplete-chip">未割当・不足：${escapeHtml(missing.join('・'))}</div>` : ''}<p><strong>${escapeHtml(areaName(schedule.areaId))}</strong>${schedule.returnDate ? ` ／ 終了 ${escapeHtml(fmtDate(schedule.returnDate))}` : ''}</p>${locations}<div class="summary-line"><b>担当者</b>${escapeHtml(assigneeSummary(schedule))}</div><div class="summary-line"><b>リソース</b>${escapeHtml(resourceSummary(schedule))}</div>${schedule.otherContent ? `<div class="summary-line"><b>内容</b>${escapeHtml(schedule.otherContent)}</div>` : ''}${schedule.otherTransport ? `<div class="summary-line"><b>場所・交通</b>${escapeHtml(schedule.otherTransport)}</div>` : ''}${schedule.memo ? `<div class="summary-line"><b>メモ</b>${escapeHtml(schedule.memo)}</div>` : ''}${customFieldDetailHtml(schedule)}${renderScheduleExtensionDetails(schedule, type)}${files}${scheduleActionButtons(schedule)}</article>`;
}
...
function renderDetail(focusId = '') {
  ...
  qsa('[data-edit-memo]', $('detailArea')).forEach((b) => b.addEventListener('click', () => openMemoForm(b.dataset.editMemo)));
